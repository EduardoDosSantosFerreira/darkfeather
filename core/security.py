...
class SecurityAnalyzer:
    ...
    def analyze_network(cls, network: WifiNetwork) -> Dict[str, Any]:
        ...
        if "open" in auth or "aberto" in auth:
            ...
            recommendations.append("Substitua WEP por WPA2 ou WPA3")
        elif "wpa" in auth:
        ...
        # 4. Análise de senha padrão (básica)
        if network.password and network.password.lower() in ["12345678", "password", "senha", "admin"]:
            risk_score += 25
    ...
    def analyze_environment(cls, networks: List[WifiNetwork]) -> Dict[str, Any]:
        ...
        for net in networks:
            ...
            if "WPA3" in flags_str:
                wpa3_networks += 1
            if "fraca" in flags_str:
                weak_passwords += 1
        
        # Calcular health score (0-100)
        # Fórmula: 100 - (high_risk * 25 + medium_risk * 10 + open_networks * 15 + wep_networks * 20) / total
        penalties = (
            high_risk * 25 +
            medium_risk * 10 +
            open_networks * 15 +
    ...
    def get_risk_color(cls, risk_level: str) -> str:
        """Retorna a cor hexadecimal para o nível de risco"""
        colors = {
            "Baixo": "#10b981",  # verde
    ...
    def get_risk_icon(cls, risk_level: str) -> str:
        """Retorna o ícone para o nível de risco"""
        icons = {
            "Baixo": "🟢",
            "Médio": "🟡",
            "Alto": "🔴"

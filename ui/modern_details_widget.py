...
from PySide6.QtWidgets import (
    ...
    QApplication, QSizePolicy, QToolButton, QScrollArea,
...
class ModernDetailsWidget(QFrame):
    ...
    def show_placeholder(self):
        ...
        text = QLabel("Selecione uma rede")
    ...
    def add_metrics_row(self):
        """Adiciona linha de métricas com mais espaço"""
        ...
        rssi_widget = self.create_metric_item("RSSI", rssi_value, "dBm", rssi_color)
        layout.addWidget(rssi_widget)
        ...
        signal_widget = self.create_metric_item("Sinal", signal_value, "%", signal_color)
        layout.addWidget(signal_widget)
        
        # Canal
        channel = self.real_data.get("channel", "N/A")
        channel_value = str(channel) if channel != "N/A" else "—"
        channel_widget = self.create_metric_item("Canal", channel_value, "", "#2563eb")
        layout.addWidget(channel_widget)
        
        # Banda
        band = self.real_data.get("band", "N/A")
        band_value = band.replace(" GHz", "") if band != "N/A" else "—"
        band_color = "#2563eb" if "2.4" in band else "#7c3aed" if "5" in band else "#db2777"
        band_widget = self.create_metric_item("Banda", band_value, "GHz", band_color)
        layout.addWidget(band_widget)
        
        # Velocidade (opcional, se disponível)
        speed = self.real_data.get("link_speed", "N/A")
        if speed != "N/A" and speed != "Não disponível":
            speed_value = speed.replace(" Mbps", "") if "Mbps" in speed else speed
            speed_widget = self.create_metric_item("Veloc.", speed_value, "Mbps", "#64748b")
            layout.addWidget(speed_widget)
        
        layout.addStretch()
        self.scroll_layout.addWidget(metrics_widget)
    
    def create_metric_item(self, label: str, value: str, unit: str, color: str) -> QWidget:
        """Cria item de métrica compacto"""
        widget = QWidget()
        ...
        if unit and value != "—":
            ...
            unit_label.setStyleSheet(f"color: {color}80; font-size: 11px; margin-top: 4px;")
        ...
        label_label.setStyleSheet("color: #475569; font-size: 11px; font-weight: 600;")
    ...
    def add_identification_section(self):
        ...
        # Grid 2 colunas com mais espaço
        grid = QGridLayout()
        ...
        for row, col, key, value in items:
            ...
            display_value = value if value != "N/A" and value != "Não disponível" else "—"
    ...
    def add_radio_section(self):
        ...
        freq_display = f"{freq} MHz" if freq != "N/A" and freq != "Não disponível" else "—"
        
        items = [
            (0, 0, "Frequência", freq_display),
            (0, 1, "PHY", self.real_data.get("phy_type", "N/A") if self.real_data.get("phy_type") != "Não disponível" else "—"),
            (1, 0, "Velocidade", self.real_data.get("link_speed", "N/A") if self.real_data.get("link_speed") != "Não disponível" else "—"),
            (1, 1, "Largura", self.format_channel_width(self.real_data.get("channel_width", "N/A"))),
        ]
    ...
    def add_ip_section(self):
        ...
        dhcp_display = "Sim" if dhcp_status == "Sim" else "Não" if dhcp_status == "Não" else "—"
    ...
    def add_credentials_section(self):
        ...
        if not self.real_data.get("password") and not self.real_data.get("password_hex"):
            return
        ...
        pass_label.setStyleSheet("color: #475569; font-size: 12px; font-weight: 600;")
        ...
        if self.real_data.get("password_hex"):
            ...
            hex_label.setStyleSheet("color: #475569; font-size: 12px; font-weight: 600;")
    ...
    def get_rssi_color(self, rssi: str) -> str:
        try:
            ...
            elif val > -65: return "#2563eb"
            elif val > -75: return "#d97706"
            else: return "#dc2626"
        except:
            return "#64748b"
    ...
    def get_signal_color(self, percent: int) -> str:
        ...
        elif percent >= 60: return "#2563eb"
        elif percent >= 40: return "#d97706"
        else: return "#dc2626"
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if "não há" not in output.lower():
                    ...
                    if name_match: data["interface_name"] = name_match.group(1).strip()
                    mac_match = re.search(r'Endereço físico\s*:\s*([0-9A-Fa-f:-]+)', output, re.MULTILINE | re.IGNORECASE)
                    if mac_match: data["interface_mac"] = mac_match.group(1).strip().upper()
                    state_match = re.search(r'Estado\s*:\s*(.+)$', output, re.MULTILINE | re.IGNORECASE)
                    if state_match: data["interface_status"] = state_match.group(1).strip()
                    guid_match = re.search(r'GUID do perfil\s*:\s*({[0-9A-F-]+})', output, re.MULTILINE | re.IGNORECASE)
                    if guid_match: data["interface_guid"] = guid_match.group(1).strip()
        except: pass
        return data
    
    def get_connection_info(self) -> Dict:
        data = {
            "bssid": "Não disponível", "channel": "Não disponível", "frequency_mhz": "Não disponível",
            "band": "Não disponível", "rssi_dbm": "Não disponível", "signal_percent": "Não disponível",
            "link_speed": "Não disponível", "phy_type": "Não disponível", "channel_width": "Não disponível"
        }
        if not self.network: return data
        try:
            result = subprocess.run(
                ["netsh", "wlan", "show", "interfaces"],
                capture_output=True, text=True, encoding='utf-8', errors='ignore'
            ...
            if result.returncode == 0:
                ...
                if current_ssid.lower() == self.network.ssid.lower():
                    ...
                    if bssid_match: data["bssid"] = bssid_match.group(1).strip().upper()
                    ...
                    if speed_match: data["link_speed"] = speed_match.group(1).replace(',', '.') + " Mbps"
                    phy_match = re.search(r'Tipo de rádio\s*:\s*(.+)$', output, re.MULTILINE | re.IGNORECASE)
                    if phy_match: data["phy_type"] = phy_match.group(1).strip()
        except: pass
        return data
    
    def get_ip_info(self) -> Dict:
        data = {
            "ipv4": "Não disponível", "ipv6": "Não disponível", "subnet_mask": "Não disponível",
            "gateway": "Não disponível", "dns_servers": "Não disponível", "dhcp_enabled": "Não disponível"
        }
        try:
            result = subprocess.run(
                ["ipconfig", "/all"],
            ...
            if result.returncode == 0:
                ...
                for section in sections:
                    if "wi-fi" in section.lower() or "wireless" in section.lower() or "wlan" in section.lower():
                        ...
                        if dhcp_match: data["dhcp_enabled"] = "Sim" if "sim" in dhcp_match.group(1).lower() else "Não"
                        ip_match = re.search(r'Endereço IPv4[ .]*:?\s*([0-9.]+)', section, re.IGNORECASE)
                        if ip_match: data["ipv4"] = ip_match.group(1)
                        ip6_match = re.search(r'Endereço IPv6[ .]*:?\s*([0-9a-f:]+)', section, re.IGNORECASE)
                        if ip6_match: data["ipv6"] = ip6_match.group(1)
                        mask_match = re.search(r'Máscara de sub-rede[ .]*:?\s*([0-9.]+)', section, re.IGNORECASE)
                        if mask_match: data["subnet_mask"] = mask_match.group(1)
                        gw_match = re.search(r'Gateway padrão[ .]*:?\s*([0-9.]+)', section, re.IGNORECASE)
                        if gw_match: data["gateway"] = gw_match.group(1)
                        dns_list = []
                        dns_matches = re.findall(r'Servidores? DNS[ .]*:?\s*([0-9.]+)', section, re.IGNORECASE)
                        for dns in dns_matches[:3]: dns_list.append(dns)
                        if dns_list: data["dns_servers"] = ", ".join(dns_list)
                        break
        except: pass
        return data
    
    def get_security_info(self) -> Dict:
        data = {"akm": "Não disponível", "pmf": "Não disponível", "wps": "Não disponível", "hidden": "Não disponível"}
        if not self.network: return data
        try:
            result = subprocess.run(
            ...
            if result.returncode == 0:
                ...
                if akm_match: data["akm"] = akm_match.group(1).strip()
                pmf_match = re.search(r'PMF\s*:\s*(.+)$', output, re.MULTILINE | re.IGNORECASE)
                if pmf_match: data["pmf"] = pmf_match.group(1).strip()
                wps_match = re.search(r'WPS\s*:\s*(.+)$', output, re.MULTILINE | re.IGNORECASE)
                if wps_match: data["wps"] = wps_match.group(1).strip()
